...
export const launchArchive = () => {
  ...
  function init(tilesList) {
    ...
    for (var i = 0; i < tilesList.length; i++) {
      ...
      const openMedia = (data) => {
        ...
        const content = document.querySelector(".media-memories");
        ...
        console.log("media region: " + data[index].region);
        console.log("media category: " + data[index].category);
      ...
      img.src =
        tilesList[i].type === "audio"
          ? "./assets/img/audio.svg"
          : tilesList[i].url;
      img.className =
        tilesList[i].type === "audio" ? "image-tile audio-svg" : "image-tile";
    ...
    drag = function (e) {
      ...
      for (var i = 0; i < elements.length; i++) {
        if (elements[i].style.transform.includes("skewY")) {
          elements[i].style.transform = elements[i].style.transform.replace(
            /skewY\([0-9]+deg\)/,
            `skewY(${skew}deg) `
          );
        } else {
          elements[
            i
          ].style.transform = `${elements[i].style.transform} skewY(${skew}deg)`;
        }
      }
      controls.update();
    };
  ...
  buttonCloseMediaModal.addEventListener("click", () => {
    ...
    while (mediaContent.children[0])
      mediaContent.removeChild(mediaContent.lastChild);
    layer.style.display = "block";
  });
