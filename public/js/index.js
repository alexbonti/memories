...
function animateTextLanding() {
  ...
  splitWords.fromTo(
    split.words,
    { y: "-60%", opacity: 0 },
    { y: "20%", opacity: 1, stagger: 0.09, scale: 1 },
    "-=1"
  );
...
function animateSlides({
  playVideoAutomatically = true,
  videoPlayerClassName = "video1",
}) {
  //Init Controller
  const video = document.querySelectorAll("video");

  if (playVideoAutomatically) video.forEach((item) => item.play());
  controller = new ScrollMagic.Controller();
...
function navToggle(e) {
  if (!e.target.classList.contains("active")) {
    ...
    gsap.to(".nav-bar", 0.75, { clipPath: "circle(4500px at 100% -10%)" });
...
barba.init({
  ...
  views: [
    ...
    },
    {
      ...
      beforeEnter() {
        logo.href = "./index.html";
        getPageName("before enter documentary");
      },
      beforeLeave() {
        controller.destroy();
        getPageName("before leave documentary");
      },
    },
    {
  ...
  if (name == "home") {
    ...
    if (language == null) {
      ...
      $.getJSON("locales/en/aboutTranslations.json", function (data) {
        $("#abouttitle1").html(data.title1);
        $("#abouttitle2").html(data.title2);
        $("#aboutpara1").html(data.para1);
      });
    } else {
      if (language == "en") {
        $.getJSON("locales/en/commonTranslations.json", function (data) {
    ...
    if (language == null) {
      ...
      $.getJSON("locales/en/contactTranslations.json", function (data) {
        $("#contacttitle1").html(data.title1);
        $("#contacttitle2").html(data.title2);
      });
    } else {
      if (language == "en") {
        $.getJSON("locales/en/commonTranslations.json", function (data) {
...
$("#enBtn").on("click", function (e) {
  ...
  } else if (pageTitle == "videostories") {
  } else if (pageTitle == "memorywalks") {
    $.getJSON("locales/en/memorywalksTranslations.json", function (data) {
      $("#title1").html(data.title1);
      $("#title2").html(data.title2);
      $("#title3").html(data.title3);
